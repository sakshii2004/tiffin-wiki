...
/**
 * On-demand secure WhatsApp number disclosure button.
 * Defends against automated phone scrapers by fetching the number
 * only upon user click via a rate-limited server endpoint.
 */
export function ShowNumberButton({ serviceId, serviceSlug }: ShowNumberButtonProps) {
  ...
  async function handleReveal() {
    ...
    try {
      const res = await fetch(`/api/listings/${serviceId}/reveal-phone`, {
        method: 'POST',
      });
      ...
      if (!res.ok) {
        if (res.status === 429) {
          ...
          setError('Could not load number. Please try again.');
        }
        return;
      }
      ...
      if (data.whatsappNumber) {
        ...
        trackEvent('WHATSAPP_REVEAL', {
          ctaName: 'WHATSAPP_REVEAL',
          listingId: serviceId,
          listingSlug: serviceSlug,
        });
      }
    } catch {
  ...
  const formatted = whatsappNumber ? formatNumber(whatsappNumber) : null;
  const waUrl = whatsappNumber ? toWaLink(whatsappNumber) : null;
      ...
      <a
        ...
        onClick={() =>
          trackEvent('WHATSAPP_OPEN', {
            ctaName: 'WHATSAPP_OPEN',
            listingId: serviceId,
            listingSlug: serviceSlug,
          })
        }
        className={cn(baseClasses, 'inline-flex items-center gap-2 font-bold w-full sm:w-auto')}
  ...
  return (
    <div className="flex flex-col gap-1 w-full sm:w-auto">
      <button
        ...
        disabled={loading}
        className={cn(baseClasses, 'flex items-center gap-2 w-full sm:w-auto')}
        aria-label="Reveal WhatsApp contact number"
      >
        {loading ? (
          <>
      ...
      {error && (
        <span className="text-xs text-red-500 font-medium px-2" role="alert">
          {error}
        </span>
      )}
    </div>
